# -*- coding: utf-8 -*-
"""
Created on Wed Jan  3 12:18:10 2024
...
#%% 找出两个.csv文件的差集
# 假设有两个 CSV 文件，分别是 file1.csv 和 file2.csv
# file1.csv代表原始数据
# file2.csv代表优化数据
file1_path = 'RAFM-dataset.csv'
file2_path = '7 Split.csv'
...
split_size = math.ceil(len(diff_df)/num_splits)
...
for i in range(num_splits):  
    ...
    addition_df = diff_df.iloc[k: k+split_size]        
    # 将抽取的数据删除，保留想保留的部分
    result_df = pd.concat([df2, addition_df], ignore_index=True)
    
    # 将result_df所有数据随机打乱
    result_df = result_df.sample(frac=1).reset_index(drop=True)
    
    # 给 result_df 第一列加一个索引 Index
...
for index, df in enumerate(list_of_dfs):
    ...
    df.to_csv(csv_filename, index=False, index_label='Index')
